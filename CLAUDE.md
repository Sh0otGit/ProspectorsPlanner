...
|---|---|---|---|
| `hb2504.utep.edu` | Instructor eval scores, per section, back to ~2019; also a real headshot per profile | none | Statute-mandated public posting. Legally clean. The real signal. Re-verified 2026-08-18, works exactly as below. Each profile also has a real photo at a predictable `/photos/{username}.jpg` (confirmed 2026-09-13 live). Not scraped/re-hosted: the Instructors page hotlinks it directly (`prototype/js/page-instructors.js`), the same "browser fetches a real third-party image itself" pattern the Map page's OSM tiles already use, since it's the same statute-mandated public source, not a new sourcing decision. Not every instructor has one uploaded -- falls back to a plain initials avatar on the image failing to load, never a guessed photo. |
