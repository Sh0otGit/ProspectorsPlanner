...
behind having a schedule. A pin is the building's real outline (done
2026-09-12) when one's on file -- `scrapers/buildingfootprints.js` matches
each building's known point against real OSM building ways from the
public Overpass API, point-in-polygon, not a name match -- falling back
to a plain colored circle at the point when no footprint has been
scraped for that building yet, the same "no data, not a guess" rule
this project applies everywhere else a lookup can come up empty. See
Data sources below for the Overpass sourcing. The view is click-and-drag (and touch-drag)
...
|---|---|---|---|
| `hb2504.utep.edu` | Instructor eval scores, per section, back to ~2019; also a real headshot per profile, plus office building/room, phone, email, bio, education, scholarly and creative activity, and grants | none | Statute-mandated public posting. Legally clean. The real signal. Re-verified 2026-08-18, works exactly as below. Each profile also has a real photo at a predictable `/photos/{username}.jpg` (confirmed 2026-09-13 live). Not scraped/re-hosted: the Instructors page hotlinks it directly (`prototype/js/page-instructors.js`), the same "browser fetches a real third-party image itself" pattern the Map page's OSM tiles already use, since it's the same statute-mandated public source, not a new sourcing decision. Not every instructor has one uploaded -- falls back to a plain initials avatar on the image failing to load, never a guessed photo. The office/contact block and the four profile tabs (`scrapers/profiles.js`'s `parseProfileDetails`, added 2026-09-13) are parsed from the same profile page `fetchInstructorProfile` already fetches for the evaluation-link table, not a second request. Each field is independently present or absent per instructor, confirmed against two real profiles (one with office/phone blank, one with a fully populated bio/education/grants) -- an empty tab renders the literal `<p class="noInfo">No info available.</p>`, which the parser treats as null, not an empty string, same "no data, not a guess" rule as the photo. Shown in a click-to-open popup off the instructor photo (`showPhotoModal` in `page-instructors.js`), alongside the photo itself. |
| `catalog.utep.edu/course-search` | Static course catalog: title, description, credit hours, prerequisites | none | A Leepfrog/CourseLeaf "FOSE" JSON API. No CRNs, no seats, no instructors, no meeting times -- catalog-year granularity, not per-term sections. Confirmed working 2026-08-18. Useful only for course descriptions/prereqs, not scheduling. The real request shape (confirmed live 2026-09-13 via CDP network capture of a real search-then-click session, same technique as RMP/Concept3D) is two POSTs to `course-search/api/?page=fose`: `route=search` with `{"other":{"srcdb":SRCDB},"criteria":[{"field":"subject","value":"CS"}]}` returns every course in a subject (`{key,code,title}`, one call per subject, not per course); `route=details` with `{"group":"key:KEY","key":"key:KEY","srcdb":SRCDB,"matched":"key:KEY"}` returns the full record, including a `description_custom` field with a free-text "Prerequisite(s):" line when the course has one. Confirmed stateless with a bare curl, no auth or cookies needed either call. `scrapers/coursecatalog.js` scrapes only `Prerequisite(s)`, shown as a gray informational notice on the Instructors page (`companionNoticesHTML` in `page-instructors.js`); stored in the new `course_catalog` table, keyed by (subject, course_number) since this source is catalog-year granularity, not per-term. `srcdb` (the catalog year, e.g. `"2026"` for 2026-2027) is a hand-maintained constant (`DEFAULT_SRCDB`) since the FOSE dropdown has no "current" flag to read programmatically -- bump it by hand once a new catalog year rolls over, same as `run.js`'s own `DEFAULT_TERM`. Not yet wired into the admin ingestion page's per-scraper buttons/auto-schedule (the other five sources all have one) -- runnable today via `node scrapers/run.js catalog`, added there deliberately as a smaller lift than the full ingestion-page plumbing until that's actually wanted. |
...
| `tile.openstreetmap.org` | Basemap imagery (real streets/building footprints) under the Map page's pins | none | Not scraped: this is OSMF's own public tile service, meant for exactly this kind of embedding -- see their tile usage policy at `operations.osmfoundation.org/policies/tiles/` (attribution, no bulk/automated fetching, browser-normal request volume). Investigated as an alternative to Concept3D's own detailed campus view, which turned out not to be a static image at all: CDP network capture of both `map.concept3d.com/?id=843` and the live `utep.edu/map/` embed (confirmed 2026-08-21) showed it's the same OSM building/street data, rendered live by MapboxGL through Concept3D's paid `tileserver.concept3d.com`, styled with a MapTiler-sourced vector style (`c3d_default_style`) -- reusing that tileserver directly would mean depending on another vendor's metered infrastructure, a real step up from the public points API above, so the public OSM tile service is used instead. Rendered as plain SVG `<image>` elements in `prototype/js/page-map.js`, positioned with the standard Web Mercator slippy-map formula, no mapping library. A "cleaner-looking basemap" alternative (CARTO Positron) was looked at 2026-09-12 and ruled out: confirmed live that its free-tier raster tiles now watermark every tile "API KEY REQUIRED" without a signed-up key, the same kind of metered-vendor dependency this project already chose OSM's own tiles specifically to avoid. Current tiles stay as they are. |
| Overpass API (`overpass-api.de`) | Real building footprint polygons for the Map page's "highlight the whole building" pins | none | Confirmed 2026-09-12: a bounding-box query around a Concept3D building point returns real OSM building ways with full ring geometry; the ring that actually contains that point (point-in-polygon, not a name match -- OSM's own building name doesn't reliably agree with Concept3D's) is the real match. A shared, volunteer-funded public service with its own fair-use policy (`wiki.openstreetmap.org/wiki/Overpass_API`), the same kind of deliberate call as Concept3D and RMP above -- one query per building, only for buildings not already matched, same "cache aggressively, backfill once" instinct as HB 2504. Response time swings widely run to run on this shared infrastructure (confirmed live, not assumed: one attempt timed out while a retry on the same endpoint moments later answered instantly) -- a single building's request failing isn't fatal to a scrape run, it just stays unmatched and gets retried next time. See `scrapers/buildingfootprints.js`. |
...
- **Five independent scrapers**, deliberately not run together, each with
  ...
  - **Building footprints** -- one Overpass query per real (non-parking)
    campus location that doesn't already have a footprint on file (~800
    on a fresh database), matching by whether the building's known point
    falls inside the returned OSM way. Runs every 120 days, same
    reasoning as campus map -- buildings don't move. The first run queries
    every building and can take a while; a location that already matched
    is never re-queried (footprints don't change), and one that didn't
    match keeps getting retried each run rather than being given up on.
    See Data sources for the Overpass sourcing and
    `scrapers/buildingfootprints.js` for the point-in-polygon match.
